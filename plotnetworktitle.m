function h = plotnetworktitle(meaM,mealimV,nodenameM,tittxt,figno,directed,maxwordlength)
% h = plotnetworktitle(meaM,mealimV,nodenameM,tittxt,figno)
...
%               figure.
% - directed  : give 0 if the connections are non-directed, otherwise they 
%               are assumed to be directed.
% - maxwordlength: give the maximum word length for the node labels, if
%               empty no limit will be applied (maxwordlength=0). 
%               Default: 0 (no limit). 
...
if isempty(maxwordlength), maxwordlength=0; end
if maxwordlength<0, maxwordlength=0; end
if isempty(directed), directed=1; end
if isempty(figno), figno = gcf; end
...
if ~((size(nodenameM,1)==1 && size(nodenameM,2)==nnode) || size(nodenameM,1)==nnode)
...
if iscell(nodenameM)
    ...
    nodenameM = [];
    for inode = 1:nnode
        nodenameM = str2mat(nodenameM,tmpC{inode});
    end
    nodenameM(1,:) = [];
end
if maxwordlength>0 && maxwordlength<size(nodenameM,2)
    nodenameM = nodenameM(:,1:maxwordlength);
end
...
if ~directed
    issymmetric = 1;
elseif nansum(nansum(abs(meaM-meaM')))<100*eps
    symyesno = input('Is the input matrix symmetric? (y/n)>','s');
    if symyesno=='y'
        issymmetric = 1;
    else
        issymmetric = 0;
